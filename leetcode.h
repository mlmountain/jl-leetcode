...
public:
     ...
     */
    int _01Backpack(int M, int N, vector<int>& weight, vector<int>& value);
    void test01Backpack();
     ...
     *
     * 分析:
     * dp[i][j] 表示从下标为[0-i]的物品里任意取，每个物品取任意个，放进容量为j的背包，价值总和最大是多少
     * dp[i][j] 有两个方向推出:
     *     1. 不放物品i: 由dp[i - 1][j]推出，即背包容量为j，里面不放物品i的最大价值，此时dp[i][j]就是dp[i - 1][j]
     *     2.   放物品i: 物品是重复的, 可以再次放入物品i, dp[i][j - weight[i]] 表示存放当前物品(第i件物品)时, 重量为j-weight[i]时的最大价值,
     *                  那么dp[i][j - weight[i]] + value[i] 就是第i件物品再次放入的最大价值
     *
     * 递推公式:
     ...
     * 输出一个整数，代表小明能够携带的研究材料的最大价值。
     */
    int completeBackpack(int M, int N, vector<int>& weight, vector<int>& value);
    void testcompleteBackpack();
